...
class WebRTCServer : public VideoSink {
 public:
   ...
   *
   * @param name Sink name (arbitrary unique identifier)
   * @param listenAddress WebSocket listen address (empty string for all
   * addresses)
   * @param port WebSocket port number
   */
  WebRTCServer(std::string_view name, std::string_view listenAddress,
               int port) {
    m_handle = CreateWebRTCServer(name, listenAddress, port, &m_status);
  }
  ...
  std::string GetListenAddress() const {
    m_status = 0;
    return cs::GetWebRTCServerListenAddress(m_handle, &m_status);
  }
  ...
  int GetPort() const {
    m_status = 0;
    return cs::GetWebRTCServerPort(m_handle, &m_status);
  }
