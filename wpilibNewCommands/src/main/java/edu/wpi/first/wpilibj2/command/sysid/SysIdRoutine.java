...
public class SysIdRoutine extends SysIdRoutineLog {
                ...
                () -> {
                  m_mechanism.drive.accept(
                      m_outputVolts.mut_replace(
                          outputSign * timer.get() * m_config.rampRate.in(Volts.per(Second)),
                          Volts));
  ...
  public Command dynamic(Direction direction) {
    ...
    return m_mechanism
        .subsystem
        .runOnce(
            () -> m_outputVolts.mut_replace(m_config.stepVoltage.in(Volts) * outputSign, Volts))
        .andThen(
            m_mechanism.subsystem.run(
                () -> {
                  m_mechanism.drive.accept(m_outputVolts);
                  m_mechanism.log.accept(this);
                  m_recordState.accept(state);
                }))
        .finallyDo(
