...
NTMecanumDriveModel::NTMecanumDriveModel(nt::NetworkTableInstance inst,
    ...
    : m_inst{inst},
      ...
      m_controllable{inst.GetBooleanTopic(std::format("{}/.controllable", path))
                         .Subscribe(0)},
      m_flPercent{
          inst.GetDoubleTopic(std::format("{}/Front Left Motor Speed", path))
              .GetEntry(0)},
      m_frPercent{
          inst.GetDoubleTopic(std::format("{}/Front Right Motor Speed", path))
              .GetEntry(0)},
      m_rlPercent{
          inst.GetDoubleTopic(std::format("{}/Rear Left Motor Speed", path))
              .GetEntry(0)},
      m_rrPercent{
          inst.GetDoubleTopic(std::format("{}/Rear Right Motor Speed", path))
              .GetEntry(0)},
